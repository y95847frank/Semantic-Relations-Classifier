...
with open('word2IDx.pickle', 'rb') as f:
    ...
    if args.train:
        filepath = "weights_CNN_best_unstable_wordemb_POS1_POS2_final.h5"
        ...
        '''
        model.load_weights('weights_CNN_best_unstable_wordemb_POS1_POS2_final.h5')
        predict = model.predict([test_pos1, test_pos2, test_token, test_lexical])
        np.save('model_3.npy', predict)
        '''
        model_1 = np.load('model_1.npy')
        model_2 = np.load('model_2.npy')
        model_3 = np.load('model_3.npy')

        final_model = np.add(np.add(model_1, model_2), model_3)
